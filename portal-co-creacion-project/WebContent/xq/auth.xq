...
updating function page:register( $new_user_data)
{
  ...
  let $name_document := concat($new_user_data//user,".xml")
   let $new := <account xmlns="">
                        <user>{$new_user_data//user/text()}</user>
                        <name>{$new_user_data//name/text()}</name>
                        <email>{$new_user_data//email/text()}</email>
                        <password>{$new_user_data//password/text()}</password>
                        <enable>true</enable>
                        <role>user</role>
                        <creation-date>{current-dateTime()}</creation-date>
                      </account>
  return if(not(db:exists("account",$name_document)))
       then (db:add("account",$new,$name_document),db:output(<rest:redirect>/static/auth/login.xml</rest:redirect>))
